...
pca <- prcomp(data1, center = TRUE)
write.csv(pca$rotation, "G:/R/finalexp/big/kmeans/01/csv/pca.csv")

screeplot(pca, npcs = 200, type = "lines", pch = 18)
grid()

pca.var <- pca$sdev ^ 2
pca.pvar <- pca.var/sum(pca.var)

plot(cumsum(pca.pvar),xlab="", ylab="", xlim = c(0,1000), ylim=c(0,1), type='b', pch = 18)
title(main = "Percentage of variance covered", cex.main = 2 , cex.lab = 1.6, xlab = "Number of Principal Components", ylab = "Sum of variances")
grid()

chosen <- 300

w <- matrix(0,nrow = dim(data)[1],ncol = chosen)
red <- pca$rotation[,1:chosen]
...
times <- sort(rep(1:timestamps,people))
w$timestamp <- times
write.csv(w,file = paste("G:/R/exp/big/kmeans/01/csv/pca",chosen,"tdm.csv",sep = ""), row.names = FALSE)


#------------------------------------------------------------
#------------------------Clustering--------------------------
#------------------------------------------------------------
w <- read.csv(paste("G:/R/exp/big/kmeans/01/csv/pca",chosen,"tdm.csv",sep = ""))
DF <- w
...
write.csv(m,"G:/R/finalexp/big/kmeans/01/finalresults.csv")
...
for(z in 1:people){
  ...
  path <- paste("G:/R/finalexp/big/kmeans/01/plots/",chosen,"/behavior/",myFile,sep = "")
  
  png(filename = path, width = 1200, height = 1200)
  plot(X,Y,type = "h", xlab = "", ylab = "", cex.axis = 1.6, pch = 18, ylim = c(0, 1))
  title(main = "Behavior",xlab = "timestamp",ylab = "behavior", cex.main = 2.5, cex.lab = 1.6)
  dev.off()
}
...
for(z in 1:people){
  ...
  myFile <- paste(z,".png",sep = "")
  path <- paste("G:/R/finalexp/big/kmeans/01/plots/",chosen,"/distance/",myFile,sep = "")
  
  png(filename = path, width = 1200, height = 1200)
  plot(X,L,type = "b", xlab = "", ylab = "", cex.axis = 1.6, pch = 18, cex = 2)
  title(main = "Activity",xlab = "timestamp",ylab = "distance", cex.main = 2.5, cex.lab = 1.6)
  dev.off()
}
