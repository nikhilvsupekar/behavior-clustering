...
write.csv(out,file = "G:/R/finalexp/big/kmeans/11/csv/person.csv")
...
DF$timestamp <- times
write.csv(DF, file = "G:/R/finalexp/big/kmeans/11/csv/tdm.csv", row.names = FALSE)
...
screeplot(pca, npcs = 140, type = "lines")
grid()

pca.var <- pca$sdev ^ 2
pca.pvar <- pca.var/sum(pca.var)

plot(cumsum(pca.pvar),xlab="Principal Components", ylab="Sum of variances", ylim=c(0,1), type='b', pch = 18)
grid()
...
times <- sort(rep(1:timestamps,people))
w$timestamp <- times
write.csv(w,file = paste("G:/R/exp/big/kmeans/11/csv/pca",chosen,"tdm.csv",sep = ""), row.names = FALSE)
...
write.csv(m,"G:/R/finalexp/big/kmeans/11/finalresults.csv")
...
for(z in 1:people){
  ...
  path <- paste("G:/R/finalexp/big/kmeans/11/plots/behavior/",myFile,sep = "")
...
for(z in 1:people){
  ...
  myFile <- paste(z,".png",sep = "")
  path <- paste("G:/R/finalexp/big/kmeans/11/plots/distance/",myFile,sep = "")
